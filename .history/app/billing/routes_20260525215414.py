    ...
    try:
        ...
        with connection.cursor() as cur:
            ...
            for item in items:
                ...
                if not stock_data:
                    connection.rollback()
                    return jsonify({'status': 'error', 'error': f"Product not found"}), 400
                
                current_stock = int(stock_data[0]) if stock_data[0] else 0
                if current_stock < qty:
                    connection.rollback()
                    return jsonify({'status': 'error', 'error': f"Insufficient stock"}), 400
        ...
        if connection:
            connection.rollback()
        print(f"MySQL Error in createbill: {str(e)}")
