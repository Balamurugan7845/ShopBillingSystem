...
def createbill_api():
    ...
    try:
        ...
        print(f"ERROR converting customer_id: {customer_id_raw}, error: {e}")
        return jsonify({'status': 'error', 'error': f'Invalid customer ID: {customer_id_raw}'}), 400
    ...
    print(f"DEBUG: Received request - customer={customer}, items={items}, subtotal={subtotal}")
    ...
    try:
        with connection.cursor() as cur:
            ...
            print(f"DEBUG: Inserting bill with customer={customer}, bill_number={bill_number}")
            cur.execute("""
